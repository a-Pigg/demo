<template>
  <div class="revert">
    <el-card class="revert-card">
      <div slot="header" class="revert-card-header">
        <ds-query-form>
          <ds-query-form-left-panel>
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item @click.native="editRevertForm">{{
                  $t("h.changeBtn")
                }}</el-dropdown-item>
                <el-dropdown-item @click.native="deleteRevertBills">{{
                  $t("h.delBtn")
                }}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
    ...
    >
      <el-form ref="filterFormRef" label-width="110px" :model="filterForm">
        <el-form-item :label="$t('h.personWhoReturned')" prop="staffId">
          <!-- <el-select
          ...
          <el-select
            ref="highFilterStaffSelectref"
            v-model="filterForm.staffId"
            :placeholder="$t('h.tips4') + $t('h.personWhoReturned')"
            multiple
            clearable
            filterable
            :filter-method="() => {}"
        ...
        <el-form-item :label="$t('h.revertDepartment')" prop="deptId">
          <ds-select-tree
            v-model="filterForm.deptId"
            :placeholder="$t('h.tips4') + $t('h.revertDepartment')"
            :multiple="true"
            :checkStrictly="true"
            :treeData="treeDepts"
        ...
        <el-form-item :label="$t('h.revertDate')" prop="date">
          <el-date-picker
    ...
    >
      <template slot="form">
        <el-form
          ...
          label-width="130px"
          :model="revertForm"
          :rules="revertFormRules"
        >
          <el-form-item
            :label="$t('h.personWhoReturned')"
            prop="staffId"
            v-if="newBillsDialogVisible"
          >
            <el-select
              v-model="revertForm.staffId"
              :placeholder="$t('h.tips4') + $t('h.personWhoReturned')"
              clearable
              filterable
              :filter-method="() => {}"
          ...
          <el-form-item :label="$t('h.revertDate')" prop="date">
            <el-date-picker
              type="date"
              value-format="yyyy-MM-dd"
              :placeholder="$t('h.tips4') + $t('h.revertDate')"
              v-model="revertForm.date"
            >
              <!-- format="yyyy 年 MM 月 dd 日" -->
            </el-date-picker>
          </el-form-item>
          ...
          >
            <!-- <el-select
              v-model="revertForm.warehouseId"
              placeholder="请选择退库后存放地点"
              filterable
              remote
              :remote-method="warehouseRemoteMethod"
            >
    ...
    <bill-print-template
      ref="vptRef"
      :title="$t('h.title14')"
      :selectId="revertTemplateId"
      :voucherPrintTempList="billPrintTemplate"
      @setVoucherTag="setBillPrintTemplate"
    ></bill-print-template>
    <bill-print-dialog
      ref="vpdRef"
      locationName="revertTemplateId"
      :voucherData="selectRevertBills"
      :userInfo="userInfo"
...
export default {
  ...
  data() {
    return {
      ...
      tableLoading: false,
      billsColums: revertBillsColumns,
      billsTableConfig: {
        selection: true,
      ...
      revertFormRules: {
        ...
        deptId: [
          {
            required: true,
            message: this.$t("h.tips3") + this.$t("h.userDepartment"),
            trigger: "change",
          },
        ],
        warehouseId: [
          {
            required: true,
            message: this.$t("h.tips3") + this.$t("h.area"),
            trigger: "change",
          },
        ],
      },
  ...
  methods: {
    getRevertBills() {
      ...
      select({
        ...
        .then(({ data }) => {
          ...
          this.revertBills = data.data[1];
          this.revertBills.forEach((item) => {
            if (item.image) {
      ...
      for (let key in this.filterForm) {
        if (this.filterForm[key] && this.filterForm[key].length) {
          if (key == "staffId") {
            ...
            highFilteritem.name = this.$t("h.personWhoReturned");
            ...
            highFilteritem.name = this.$t("h.revertDepartment");
            highFilteritem.keyStr = key;
            highFilteritem.value = labels.join(",");
            this.highFilterTags.push(highFilteritem);
          } else if (key == "status") {
            ...
            highFilteritem.name = this.$t("h.afterReturningInventory");
            highFilteritem.keyStr = key;
            highFilteritem.value = labels.join(",");
            this.highFilterTags.push(highFilteritem);
          } else if (key == "date") {
            let highFilteritem = {};
            highFilteritem.name = this.$t("h.revertDate");
            highFilteritem.keyStr = key;
    ...
    newBillsDialogOpened() {
      this.$refs.revertFormRef.clearValidate();
    ...
    newBillsDialogClose() {
      ...
      this.billAssets = [];
      this.revertForm._id = "";
      this.revertForm.date = "";
      this.revertForm.deptId = "";
      this.revertForm.staffId = "";
      this.revertForm.warehouseId = "";
      this.newBillTitle = this.$t("h.title12");
      this.$refs.billsTableRef.$refs.dsTableRef.clearSelection();
    },
    sbumitRevertForm() {
      this.$refs.revertFormRef.validate((valid) => {
        ...
        let permissonId = this.revertForm._id == "" ? 29 : 61;
        let assetIdsArr = [];
        let assetNamesArr = [];
        let assertCodesArr = [];
        let fromStateArr = [];
        ...
        update({
          ...
          token: this.userInfo.token,
          requstData: {
            _id: this.revertForm._id, //为空新增，不为空修改
            assetIds: assetIdsArr.join(","), //assetIds
            assetNames: assetNamesArr.join(","),
            assertCodes: assertCodesArr.join(","),
            deptId: this.revertForm.deptId, //部门id
            staffId: this.revertForm.staffId, //员工id
            date: this.revertForm.date, //退库日期
            wareHouseId: this.revertForm.warehouseId,
            returnState: "0",
            fromState: fromStateArr.join(","), //  之前的状态
    ...
    deptInput(val) {
      ...
      if (matchStaffData.length) {
        let matchStaff = matchStaffData.find(
          (item) => item._id == this.revertForm.staffId
        );
        if (!matchStaff) this.revertForm.staffId = "";
        return;
      } else {
        this.revertForm.staffId = "";
      }
    ...
    staffSelectChange(val) {
      if (!val) return false;
      this.revertForm.deptId = this.staffData.find(
        (item) => item._id == val
      ).deptId;
    },
    ...
    editRevertForm() {
      if (!this.selectRevertBills.length || this.selectRevertBills.length > 1) {
        return this.$message.error(this.$t("h.tips110"));
      }
      if (this.selectRevertBills[0].status != -1) {
        return this.$message.error(this.$t("h.tips111"));
      }
      // this.getMatchEditStaff(this.selectRevertBills[0].staffId);
      this.getMatchEditAssets(this.selectRevertBills[0].assetIds);
      // this.getMatchEditWare(this.selectRevertBills[0].wareHouseId);
      this.revertForm._id = this.selectRevertBills[0]._id;
      this.revertForm.deptId = this.selectRevertBills[0].deptId;
      this.revertForm.staffId = this.selectRevertBills[0].staffId;
      this.revertForm.date = this.selectRevertBills[0].date;
      this.revertForm.warehouseId = this.selectRevertBills[0].wareHouseId;
    ...
    deleteRevertBills() {
      if (this.selectRevertBills.length == 0) {
        return this.$message.error(this.$t("h.tips22"));
      } else {
        for (let i = 0; i < this.selectRevertBills.length; i++) {
          if (this.selectRevertBills[i].status != -1) {
            return this.$notify.error({
              title: this.$t("h.title27"),
    ...
    multiDeleteBill(index) {
      if (index > this.selectRevertBills.length - 1) {
        this.paginationForm.currentPage = 1;
        this.getRevertBills();
        this.$refs["billsTableRef"].$refs["dsTableRef"].clearSelection();
      ...
      deleteData({
        ...
        token: this.userInfo.token,
        requstData: {
          _id: this.selectRevertBills[index]._id,
          permissonId: 30,
        },
      })
        .then(({ data }) => {
          if (data.code != 0) {
            this.deleteErrorData.push({
              roleRule: this.selectRevertBills[index].roleRule,
              assetNames: this.selectRevertBills[index].assetNames,
              reason: data.data,
            });
          }
    ...
    getBillPrintTemplate() {
      select({
        ...
        .then(({ data }) => {
          ...
          console.log(this.billPrintTemplate[0]._id);
          this.revertTemplateId = localStorage.getItem("revertTemplateId");
          if (
            !this.billPrintTemplate.some(
              (item) => item._id == this.revertTemplateId
            )
          ) {
            this.revertTemplateId = this.billPrintTemplate[0]._id;
            localStorage.setItem("revertTemplateId", this.revertTemplateId);
          }
        })
        .catch(() => {
    ...
    setBillPrintTemplate(id) {
      this.revertTemplateId = id;
      window.localStorage.setItem("revertTemplateId", id);
    },
    goPrintBills() {
      if (this.selectRevertBills.length == 0)
        return this.$message.error(this.$t("h.tips121"));
      if (this.selectRevertBills.length > 10)
