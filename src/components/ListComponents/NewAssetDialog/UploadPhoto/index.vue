<template>
  <div class="upload">
    ...
    <div v-show="fileValue" class="upload-img">
      <button class="close-btn" @click="deletePhotpFile">
...
export default {
  ...
  methods: {
    handlePhotoChange(e) {
      ...
      } else if (this.maxLimit.pictureSize) {
        if (file.size > this.maxLimit.pictureSize * 1024) {
          return this.$message.error(`${this.$t("h.tips19")}${this.maxLimit.pictureSize}kb`);
        }
      }
      fileToBase64(file)
        .then((res) => {
