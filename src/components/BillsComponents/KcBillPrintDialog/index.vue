...
<script>
import { printPost, printGet, update } from "@/assets/api";
  ...
  methods: {
    ...
    startPrint() {
      this.$refs["printFormRef"].validate(async (valid) => {
        ...
        this.loading = true;
        console.log(1, this.pagMultipleSelection);
        ...
        printGet({ code: 4, param: 3, key: this.facilityKey })
          .then((result) => {
            if (result.data.statInfo.devStat != "Ok") {
              ...
              console.log("this.body.printLab", this.body.printLab);
              console.log(
    ...
    async createVoucherTemplate(i, dataJSON) {
      ...
      let printAssetData = [];
      printAssetData = this.voucherData[i].list;
