        ...
        <div class="box-2" v-loading="refreshLoading">
          ...
          >
            <el-form-item :label="$t('h.formTips1')" prop="printName">
              ...
              <el-popover
                placement="top-start"
                :title="$t('h.formTips76')"
                width="200"
                trigger="hover"
              >
                <div>
                  {{ $t("h.formTips77") }}:{{ printerName }}<br />{{
                    $t("h.formTips78")
                  }}:{{
                    printerType == "RFID"
                      ? "RFID" + $t("h.formTips1")
            ...
            <el-collapse v-model="activeNames" accordion>
              ...
              >
                ...
                <el-form-item :label="$t('h.acessPassword')" prop="oldResvSect">
                  <el-input
                    :placeholder="$t('h.tips3') + $t('h.acessPassword')"
...
export default {
  data() {
    var checkUserData = (rule, value, callback) => {
      ...
      callback(new Error(this.$t("h.tips188")));
    };
    var checkResvSect = (rule, value, callback) => {
      ...
      callback(new Error(this.$t("h.tips189")));
    };
    return {
      tableData: [],
      // 导出打印结果dialog
      ...
      exportFormRules: {
        fileName: [
          {
            required: true,
            message: this.$t("h.tips3") + this.$t("h.formTips16"),
            trigger: "blur",
          },
        ],
      },
      ...
      ruleFormRules: {
        ...
        moban: [
          {
            required: true,
            message: this.$t("h.tips4") + this.$t("h.formTips17"),
            trigger: "change",
          },
        ],
        userData: [
          {
            required: false,
            message: this.$t("h.tips3") + this.$t("h.formTips18"),
            trigger: "blur",
          },
  ...
  methods: {
    ...
    async getAssetTagsTemplate() {
      ...
      // console.log(1, this.assetTagsTemplate);
    },
    // 关闭弹窗的回调
    lookClose() {
    ...
    print() {
      ...
      if (this.printerType == "RFID") {
        // console.log("printSelectList", this.printSelectList);
        for (let i = 0; i < this.printSelectList.length; i++) {
          let printAmount = this.selectAssetsData[i].printAmount
            ? Number(this.selectAssetsData[i].printAmount)
            : 0;
          this.body.printLab[i] = {
            ...
            lockLab: 0, //是否锁标签（防止修改写入芯片的信息），0：不锁标签，1：锁标签
            encodeLab: 0, //是否对写入标签芯片的EPC数据进行编码（编码后需解码方可看到系统记录的EPC），0：不编码，1：进行编码
            startIndex: printAmount, //EPC的标号，对于资产，此处传0，对于物料，此处传本次打印的起始标号
            labAmount: this.ruleForm.printNum, //需打印多少张标签（用于物料标签打印，可一个EPC编码打印多个标签；资产为一物一码，数量应为1）
          ...
          warehouseOperate({
            ...
            token: this.userInfo.token,
            requstData: {
              substanceId: this.selectAssetsData[i].substanceId,
              printAmount: printAmount + this.ruleForm.printNum,
            },
          })
            .then(({ data }) => {
    ...
    openPrint() {
      ...
      this.ruleForm.moban = parseInt(
        window.localStorage.getItem("inventoryTemaplateId")
      );
      if (
        ...
        .then(async (result) => {
          if (id == 2) {
            ...
            for (let i = 0; i < this.resultList.length; i++) {
              for (let j = 0; j < this.selectAssetsData.length; j++) {
                if (
                  this.resultList[i].epc.slice(0, 12) ==
                  this.selectAssetsData[j].epc
                ) {
                  this.resultList[i].name = this.selectAssetsData[j].name;
                }
              }
            }
            if (isEnd) {
              ...
              for (let i = 0; i < this.resultList.length; i++) {
                for (let k in this.resultList[i]) {
                  if (this.exportPrintResultData[i]) {
    ...
    exportPrintResult() {
      this.$refs["exportFormRef"].validate((vaild) => {
        ...
        const headStr = `序号,物品编码,物品名称,物品EPC,标签TID,校验码\n`;
